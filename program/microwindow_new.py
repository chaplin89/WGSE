...
# Column titles
# format: TSV (tab-separated), qCSV (quoted, comma separated), uCSV (unquoted, comma separated)
# line_end: use actual characters (\r\n or Windows vs \n or *nix; no \r or MacOS)
# (sort) order: coord (1-22, X, Y, MT), vers (1-22, MT, X, Y), alpha (1, 10, 11 ...19, 2, ..., MT, X, Y)
# Use '-' for PAR label if not used; when PAR is not used, the X PAR region is diploid (meaning the Y PAR is empty)
tmp = range(40)
kit_ext, kit_format, kit_line_end, kit_order, kit_auto_label, kit_X_label, kit_Y_label, kit_M_label, \
    kit_PAR_label, kit_PAR_after, kit_unk_value, kit_allele_cols, kit_use_haploid, kit_build, \
    kit_max_entries_K, *tmp = tmp
del tmp

# IMPORTANT: key name entries must match exactly as used in Button definition and aconv.py; order is not important
kits = {
    # all multi- / combined- file kits (vendor, Ancient, WGSE) -- all the same format as 23andMe
    'AllSNPs':          ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    'CombinedKit_v3':   ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    'CombinedKit_v5':   ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    '23andMe_V35':      ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    '23andMe_SNPs_API': ['.txt', 'TSV', '\r\n', 'vers',  'chr',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    '1240K+HO':         ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT',  '-',  '-', '--', 1, 'Hap', 37 ],
    
    # Vendor kits (genealogy)
    '23andMe_V3':       ['.txt', 'TSV', '\r\n', 'vers',  'num',  'X',  'Y', 'MT', '--',  '-', '--', 1, 'Hap', 37 ],
    '23andMe_V4':       ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT', '--',  '-', '--', 1, 'Hap', 37 ],
    '23andMe_V5':       ['.txt', 'TSV', '\r\n', 'coord', 'num',  'X',  'Y', 'MT', '--',  '-', '--', 1, 'Hap', 37 ],

    'Ancestry_V1':      ['.txt', 'TSV', '\r\n', 'coord', 'num', '23', '24',  '-', '25',  'Y',  '0', 2, 'Dip', 37 ],
    'Ancestry_V2':      ['.txt', 'TSV', '\r\n', 'coord', 'num', '23', '24', '26', '25',  'Y',  '0', 2, 'Dip', 37 ],

    # FTDNA allows both 36 and 37 download; 1-22 only, X only and combined. Some combined replicate the header before X
    'FTDNA_V1_Affy':    ['.csv', 'uCSV', '\r\n', 'coord', 'num',  'X',  '-',  '-',  '-', '-', '--', 1, '', 37 ],
    'FTDNA_V2':         ['.csv', 'uCSV', '\r\n', 'coord', 'num',  'X',  '-',  '-',  '-', 2, 'Hap', 'InDel', '0', 37 ],
    'FTDNA_V3':         ['.csv', 'qCSV', '\r\n', 'coord', 'num',  'X',  '-',  '-', 'XY', '22', '--', 2, 'Hap', 'InDel', '0', 37 ],
    ...
    # Others (more minor, newer or primarily health and wellness)
    "MTHFRGen":         ['.txt',  'TSV', '\r\n',  'vers', 'num',  'X',  'Y', 'MT',  '-', '-', '--', 1, '', 37, 640 ],
    "TellMeGen":        ['.csv',  'TSV', '\r\n', 'alpha', 'num',  'X',  'Y', 'MT', 'XY', 'X', '--', 1, '', 37, ],
    "Genera":           ['.csv', 'uCSV',   '\n', 'coord', 'num',  'X',  'Y', 'MT',  '-', '-', '--', 1, '', 37, ],
    "meuDNA_v1":        ['.csv', 'uCSV', '----', 'coord', 'num',  'X',  'Y', 'MT',  '-', '-', '--', 1, '', 37, ],
    "meuDNA_v2":        ['.csv', 'uCSV', '----', 'coord', 'num',  'X',  'Y', 'MT',  '-', '-', '--', 1, '', 37, ],
    "SelfDecode":       ['.txt',  'TSV', '----', 'coord', 'num',  'X',  'Y', 'MT',  '-', '-', '--', 1, 'Hap', 38, ],
    
    # Ancient DNA (no example files. just content listing. so use 23andMe format for now)
    "1240K":            ['.txt', 'TSV', '\r\n', 'coord', 'num', 'X', 'Y', 'MT', '--', '-', '-', 1, 'Hap', 37 ],
    "HOv1":             ['.txt', 'TSV', '\r\n', 'coord', 'num', 'X', 'Y', 'MT', '--', '-', '-', 1, 'Hap', 37 ],
}
...
def button_select_recommended():
    ...
    button_deselect_all()
    combinedv3Button.select()   ;  checkbuttons_results[kits.index("CombinedKit_v3")] = True
...
def click_checkbox(kit):
    global checkbuttons_results, generateSelectedFilesButton, kits, allSNPsButton
...
def button_generate_selected():
    ...
    for i in range(stop):
        ...
        out_oFPB = f'{out_oFPB}_{kits[i]}'        # Only generate and keep compressed form now
        if i != kits.index("AllSNPs") and checkbuttons_results[i] and \
           not _check_valid_microarray(out_oFPB, refVCFtab_qFN):
            ...
            commands += (
                ...
                f'{pythonx} {aconv} {kits[i]} "{out_FPB_AllSNPs}.txt" "{out_FPB}" "{wgse.reflib.cma_FP}"\n'
    ...
    if not (reused or checkbuttons_results[kits.index("AllSNPs")]) or \
